...
class UWSGIEnvironment():
    def __init__(self, base_dir, venv_dir, socket_dir, log_dir):
        self.base_dir = common.load_var(base_dir)
        self.virtualenv_dir = common.load_var(venv_dir)
        self.socket_dir = common.load_var(socket_dir)
        self.log_dir = common.load_var(log_dir)
...
def load_uwsgi_environments(yaml_config):
    ...
    for env_name in config_section:
        ...
        virtualenv_dir = config_section[env_name]['virtualenv_directory']
        socket_dir = config_section[env_name]['socket_directory']
        log_dir = config_section[env_name]['log_directory']

        new_env = UWSGIEnvironment(base_dir, virtualenv_dir, socket_dir, log_dir)
...
if __name__ == '__main__':
    ...
    if not env and len(env_table.keys()) > 1:
        raise Exception('The uWSGI config in file %s contains multiple environments. Please specify a target environment.' % args['<configfile>'])
